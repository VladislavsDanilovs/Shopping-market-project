...
export class CheckoutComponent implements OnInit {
  ...
  onSubmit() {
    ...
    //-short way of doing the same using functional programming
    ...
    /*Get the state and country objects 
    NOTE: JSON.parse(JSON.stringify(object)) this is simple one liner to deep clone an object, however this approach may cause data loss, but since we are not using: Dates, functions, undefined, Infinity, RegExps, Maps, Sets, Blobs, FileLists, 
    ImageDatas, sparse Arrays, Typed Arrays or other complex types in our object this is not an issue*/
            ...
            .then(function(result) {
              if (result.error) {
                ...
                this.checkoutService.placeOrder(purchase).subscribe({
                  ...
                  error: err => {
                    alert(`There was en error: ${err.message}`);
                  }
                })
              }
            }.bind(this));
